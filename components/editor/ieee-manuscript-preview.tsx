...
export function IeeeManuscriptPreview({
  ...
  const generatedFiguresBySection = new Map<string, GeneratedRenderedFigure[]>();
  const generatedTablesBySection = new Map<string, GeneratedRenderedFigure[]>();
  ...
  generatedFigures
    .filter((figure) => figure.render_success)
    .forEach((figure, index) => {
      figureNumberById.set(
        figure.spec.id,
        figure.spec.figure_number ?? figures.length + index + 1,
      );
      const bucket = generatedFiguresBySection.get(figure.spec.section) ?? [];
      bucket.push(figure);
      generatedFiguresBySection.set(figure.spec.section, bucket);
    });

  generatedTables
    .filter((table) => table.render_success)
    .forEach((table) => {
      const bucket = generatedTablesBySection.get(table.spec.section) ?? [];
      bucket.push(table);
      generatedTablesBySection.set(table.spec.section, bucket);
    });
          ...
          <div className="space-y-6 lg:columns-2 lg:gap-8 lg:space-y-0">
            {preview.sections.map((section) => (
              ...
              >
                ...
                {(generatedFiguresBySection.get(section.id) ?? []).map(
                  (figure) => (
                    <figure
                      key={figure.spec.id}
                      className="mt-4 break-inside-avoid-column rounded-xl border border-slate-200 bg-slate-50 p-3"
                    >
                      ...
                      <figcaption className="mt-2 text-center text-[9pt] text-slate-700">
                        <span className="font-semibold">
                          Fig. {figure.spec.figure_number ?? figureNumberById.get(figure.spec.id)}.
                        </span>{" "}
                ...
                {(generatedTablesBySection.get(section.id) ?? []).map((table) => {
                        ...
                        <table className="w-full border-collapse text-left text-[9pt] text-slate-800">
                          ...
                          <tbody>
                            {rows.map((row, rowIndex) => (
                              <tr key={`${table.spec.id}-${rowIndex}`} className="border-b border-slate-200">
                                {row.map((cell, cellIndex) => (
                                  <td key={`${table.spec.id}-${rowIndex}-${cellIndex}`} className="px-2 py-1">
                                    {cell}
                                  </td>
                                ))}
                              </tr>
