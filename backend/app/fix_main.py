...
from app.api.routes.internal_fix import router as internal_fix_router
...
configure_logging()
logger = logging.getLogger("papereasy.fix_service")
...
async def lifespan(_app: FastAPI):
    ...
    get_object_storage()
    logger.info("Fix service ready with persistence backend: %s", settings.persistence_backend)
    yield


app = FastAPI(title=f"{settings.app_name} Fix Service", debug=settings.debug, lifespan=lifespan)
...
async def log_requests_and_handle_errors(request: Request, call_next):
    ...
    try:
        ...
        logger.exception(
            "Unhandled fix service error while serving %s %s",
            request.method,
            request.url.path,
        )
        return JSONResponse(
...
app.include_router(internal_fix_router)
